...
Route::get('/test-email', function () {
    ...
    |----------------------------------------------------------------------
    | 0. Clears config cache so fresh env vars are used.
    | 1. Shows RAW env vars + resolved mail configuration.
    ...
    */
    ...
    // ── 0. Force-clear config cache ──────────────────────────────────
    $cacheFile = base_path('bootstrap/cache/config.php');
    $hadCache  = file_exists($cacheFile);
    \Illuminate\Support\Facades\Artisan::call('config:clear');
    // Re-read config fresh after clearing
    app()->make('config')->set(require base_path('config/mail.php'));
    // Actually we need to reload the whole config; simplest way:
    $output .= "<b style='color:#89b4fa;'>═══ CACHE STATUS ═══</b>\n";
    $output .= "Config cache existed: " . ($hadCache ? '<b style="color:#f38ba8;">YES (was stale!)</b>' : '<b style="color:#a6e3a1;">NO</b>') . "\n";
    $output .= "Ran config:clear: <b style='color:#a6e3a1;'>✓</b>\n\n";

    // ── 1a. Show RAW env vars ────────────────────────────────────────
    $output .= "<b style='color:#89b4fa;'>═══ RAW ENV VARS ═══</b>\n";
    $output .= "MAIL_MAILER       = " . (env('MAIL_MAILER') ?: '(not set)') . "\n";
    $output .= "MAIL_HOST         = " . (env('MAIL_HOST') ?: '(not set)') . "\n";
    $output .= "MAIL_PORT         = " . (env('MAIL_PORT') ?: '(not set)') . "\n";
    $output .= "MAIL_SCHEME       = " . (env('MAIL_SCHEME') ?: '(not set)') . "\n";
    $output .= "MAIL_ENCRYPTION   = " . (env('MAIL_ENCRYPTION') ?: '(not set)') . "\n";
    $output .= "MAIL_USERNAME     = " . (env('MAIL_USERNAME') ? substr(env('MAIL_USERNAME'), 0, 6) . '...' : '(not set)') . "\n";
    $output .= "MAIL_PASSWORD     = " . (env('MAIL_PASSWORD') ? '****' . substr(env('MAIL_PASSWORD'), -4) : '(not set)') . "\n";
    $output .= "MAIL_FROM_ADDRESS = " . (env('MAIL_FROM_ADDRESS') ?: '(not set)') . "\n";
    $output .= "MAIL_FROM_NAME    = " . (env('MAIL_FROM_NAME') ?: '(not set)') . "\n\n";

    // ── 1b. Show resolved mail config ────────────────────────────────
    $cfg = config('mail');
    ...
    $output .= "<b style='color:#a6e3a1;'>═══ RESOLVED CONFIG (mailer: {$mailerName}) ═══</b>\n";
    ...
    if ($encryptionEnv && !$schemeEnv) {
        ...
        $output .= "   → Add <b>MAIL_SCHEME=tls</b> to your Railway env vars.\n\n";
    }

    // ── 3. Attempt synchronous send ──────────────────────────────────
    $recipient = 'trincoabishigan@gmail.com';
...
Route::prefix('facilitator')
    ...
    Route::post('/profile/send-otp', [FacilitatorController::class, 'sendProfileOTP'])->name('facilitator.profile.send-otp');
    Route::post('/profile/verify-otp', [FacilitatorController::class, 'verifyProfileOTP'])->name('facilitator.profile.verify-otp');

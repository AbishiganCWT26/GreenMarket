...
@section('styles')
<link rel="stylesheet" href="{{ asset('css/lead_farmer/Inventory-Dashboard.css') }}">
<style>
/* Search Container */
.search-container{
    display:flex;
...
<div class="inventory-app">
	...
	<div class="stats-container">
		...
		<div class="stat-item">
			...
			<a href="{{ route('lf.inventory.list', ['status' => 'Low Stock']) }}" class="stat-link">
		...
		<div class="stat-item">
			...
			<a href="{{ route('lf.inventory.list', ['status' => 'Out of Stock']) }}" class="stat-link">
				<i class="fas fa-arrow-right"></i>
			</a>
			@endif
		</div>
				...
				<div class="card-grid" id="productCardGrid">
					...
					<div class="product-card" data-id="{{ $product->id }}"
						...
						<div class="card-footer">
							<button class="update-btn" onclick="openUpdateModal({{ json_encode([
								'id' => $product->id,
							...
							<tr class="product-row" data-id="{{ $product->id }}"
								...
								<td>
									<div class="table-date">
										<span>{{ $product->updated_at->format('d M') }}</span>
										<small>{{ $product->updated_at->diffForHumans() }}</small>
									</div>
								</td>
								<td>
									<button class="icon-btn update" onclick="openUpdateModal({{ json_encode([
										'id' => $product->id,
										'name' => $product->product_name,
										'qty' => $product->quantity,
										'unit' => $product->unit_of_measure
	...
	<div class="tab-content" id="logsTab">
		<div class="filter-wrapper">
			...
			<div class="filter-actions">
				...
				<select id="logFarmerFilter" class="filter-select">
					<option value="">All Farmers</option>
					@foreach($farmers ?? [] as $farmer)
					<option value="{{ $farmer->id }}">{{ $farmer->name }}</option>
					@endforeach
				</select>
				<a href="{{ route('lf.inventory.report.movement', ['export' => 'pdf']) }}" class="pdf-btn">
			...
			<div class="card-view active" id="logCardView">
				<div class="card-grid" id="logCardGrid">
					...
					<div class="log-card" data-id="{{ $log->id }}"
						 ...
						 data-farmer="{{ strtolower($log->product->farmer->name ?? '') }}"
						 data-farmer-id="{{ $log->product->farmer_id ?? '' }}"
						 data-type="{{ $log->type }}">
						<div class="card-image">
						...
						<div class="card-content">
							...
							<div class="card-details">
								<div class="detail-item">
									<i class="fas fa-user"></i>
									<span>{{ $log->product->farmer->name ?? 'N/A' }}</span>
								</div>
								<div class="detail-item">
					...
					<table class="data-table">
						...
						<tbody id="logTableBody">
							...
							<tr class="log-row" data-id="{{ $log->id }}"
								...
								<td>
									<div class="table-date">
										<span>{{ $log->created_at->format('d M') }}</span>
	...
	<div class="modal-overlay" id="updateStockModal">
		<div class="modal-container">
			...
			<form id="stockUpdateForm" class="modal-body">
				@csrf
				<input type="hidden" name="product_id" id="modalProductId">
				<input type="hidden" name="update_type" id="modalUpdateType" value="add">

				<div class="current-stock-info">
					<span class="label">Current Stock</span>
					<span class="value" id="modalCurrentQty">0</span>
				</div>

				<div class="type-selector">
					<button type="button" class="type-option active" data-type="add" onclick="updateType(this, 'add')">
						<i class="fas fa-plus"></i> Add
					</button>
					<button type="button" class="type-option" data-type="reduce" onclick="updateType(this, 'reduce')">
						<i class="fas fa-minus"></i> Reduce
					</button>
					<button type="button" class="type-option" data-type="adjust" onclick="updateType(this, 'adjust')">
				...
				<div class="form-group">
					...
					<input type="number" step="0.01" min="0" name="quantity" id="quantity" class="form-input" required>
					<span class="input-unit" id="modalUnit">kg</span>
				</div>

				<div class="form-group">
					<label class="form-label">Reason</label>
					<textarea name="reason" id="reason" class="form-input" rows="2" required></textarea>
				</div>

				<div class="modal-footer">
					<button type="button" class="btn-secondary" onclick="closeUpdateModal()">Cancel</button>
					<button type="submit" class="btn-primary" id="submitUpdateBtn">
...
function initData() {
	...
	productItems = [];
	// We want one object per product ID to keep views in sync
	...
	productRows.forEach(r => {
		...
		if (productMap.has(id)) {
			...
			productMap.set(id, {
				id: id,
				name: r.dataset.name,
				farmer: r.dataset.farmer,
				farmerId: r.dataset.farmerId,
...
function setupListeners() {
	...
	document.getElementById('exportProductPdf')?.addEventListener('click', function() {
		...
		let url = "{{ route('lf.inventory.report.products') }}?";
		if (status) url += "status=" + encodeURIComponent(status) + "&";
		if (farmer) url += "farmer_id=" + encodeURIComponent(farmer) + "&";
	...
	document.getElementById('logFarmerFilter')?.addEventListener('change', filterLogsFromControls);
...
function filterProducts(term) {
	const status = document.getElementById('statusFilter').value;
	const farmerId = document.getElementById('farmerFilter').value;
	const categoryId = document.getElementById('categoryFilter').value;

	filteredProducts = productItems.filter(item => {
		let match = true;
		if (term && !item.name.includes(term) && !item.farmer.includes(term)) match = false;
		if (status && item.status !== status) match = false;
		if (farmerId && item.farmerId !== farmerId) match = false;
...
function resetProductFilters() {
	...
	document.getElementById('farmerFilter').value = '';
...
function filterLogs(term) {
	...
	const farmerId = document.getElementById('logFarmerFilter').value;

	filteredLogs = logItems.filter(item => {
		let match = true;
		if (term && !item.product.includes(term) && !item.farmer.includes(term)) match = false;
...
function updateDisplay(type) {
	if (type === 'product') {
		...
		// Show current page for current view
...
function updatePagination(type, total, perPage, current) {
	...
	const container = document.getElementById(type + (type === 'product' ? (currentProductView === 'card' ? 'CardPager' : 'TablePager') : (currentLogView === 'card' ? 'CardPager' : 'TablePager')));
...
function openUpdateModal(product) {
	...
	document.getElementById('modalCurrentQty').innerText = product.qty + ' ' + product.unit;
	...
	document.getElementById('modalUpdateType').value = 'add';
...
function updateType(btn, type) {
	...
	document.getElementById('quantityLabel').innerText = labels[type];
}
...
document.getElementById('stockUpdateForm')?.addEventListener('submit', async function(e) {
	...
	try {
		...
		const response = await fetch("{{ route('lf.inventory.updateStock') }}", {
			method: 'POST',
			headers: {

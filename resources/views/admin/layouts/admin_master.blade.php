...
<body>
	...
	<div class="dashboard-wrapper">
		<aside class="sidebar" id="sidebar">
			...
			<nav class="sidebar-nav">
				<ul class="main-menu">
					<li><a href="{{ url('/admin/dashboard') }}" class="menu-link {{ request()->is('admin/dashboard') ? 'active' : '' }}"><i class="fa-solid fa-house"></i><span>Dashboard</span></a></li>
			...
			<header class="navbar">
				...
				<div class="header-right-group">
					...
					<a href="{{ url('/admin/profile') }}" class="profile-photo-link" title="View Profile">
						<img src="{{ asset('uploads/profile_pictures/' . (Auth::user()->profile_photo ?? 'default-avatar.png')) }}" class="profile-photo" data-default-src="{{ asset('uploads/profile_pictures/default-admin.png') }}" onerror="this.src=this.dataset.defaultSrc; this.onerror=null;" alt="Admin Profile">
					</a>
	...
	@php
		$swalMasterConfigData = [
			'mark_read' => file_exists(public_path('assets/icons/Gif/mark as read1.gif')) ? ['imageUrl' => asset('assets/icons/Gif/mark as read1.gif'), 'imageWidth' => 60, 'imageHeight' => 60] : ['icon' => 'success'],
			'mark_read_fail' => file_exists(public_path('assets/icons/Gif/mark as read fail1.gif')) ? ['imageUrl' => asset('assets/icons/Gif/mark as read fail1.gif'), 'imageWidth' => 60, 'imageHeight' => 60] : ['icon' => 'error'],
			...
			'alert2' => file_exists(public_path('assets/icons/Gif/alert2.gif')) ? ['imageUrl' => asset('assets/icons/Gif/alert2.gif'), 'imageWidth' => 60, 'imageHeight' => 60] : ['icon' => 'warning'],
			'validation_error' => file_exists(public_path('assets/icons/Validation Error.png')) ? ['imageUrl' => asset('assets/icons/Validation Error.png'), 'imageWidth' => 60, 'imageHeight' => 60] : ['icon' => 'error'],
			'session' => [
				'success' => session('success'),
				'error' => session('error'),
				'warning' => session('warning'),
			],
			'errors' => $errors->any() ? implode('<br>', $errors->all()) : null,
		];
	@endphp
	<script type="application/json" id="swal-master-config-data">
{!! json_encode($swalMasterConfigData) !!}
	</script>
	<script>
		const swalMasterConfig = JSON.parse(document.getElementById('swal-master-config-data').textContent);
		function getMasterSwalIcon(key) {
			return swalMasterConfig[key] || { icon: 'info' };
		}
		...
		document.addEventListener('DOMContentLoaded', function() {
			...
			if (markAllReadBtn) {
				markAllReadBtn.addEventListener('click', function() {
					...
					.catch(() => {
						Swal.fire({
							...getMasterSwalIcon('mark_read_fail'),
							title: 'Error',
			...
			if (swalMasterConfig.session.success) {
				Swal.fire({
					...getMasterSwalIcon('success2'),
					title: 'Success!',
					text: swalMasterConfig.session.success,
					timer: 3000,
					showConfirmButton: false,
					toast: true,
					position: 'top-end',
			...
			if (swalMasterConfig.session.error) {
				Swal.fire({
					...getMasterSwalIcon('error2'),
					title: 'Error!',
					text: swalMasterConfig.session.error,
					timer: 3000,
					showConfirmButton: false,
					toast: true,
					position: 'top-end',
			...
			if (swalMasterConfig.session.warning) {
				Swal.fire({
					...getMasterSwalIcon('alert2'),
					title: 'Warning!',
					text: swalMasterConfig.session.warning,
					timer: 3000,
					showConfirmButton: false,
					toast: true,
					position: 'top-end',
			...
			if (swalMasterConfig.errors) {
				Swal.fire({
					...getMasterSwalIcon('validation_error'),
					title: 'Validation Error',
					html: swalMasterConfig.errors,
					timer: 4000,
					customClass: { popup: 'swal-compact' }
				});
			}
